...
export const modifyVersion = async (projectPath, externalKeyMaps) => {
...
export const build = (projectPath) => {
...
export const modifyDist = async (projectPath) => {
  const distPath = path.join(projectPath, 'dist');
  const files = await readdir(distPath);
  const filterFiles = files?.filter(file => ['.js', '.mjs']?.includes(path.extname(file)));
...
export const publish = async (projectPath) => {
  return new Promise((resolve, reject) => {
    let error = '';
